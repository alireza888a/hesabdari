...
import { Transaction, Invoice, Customer, TransactionType, InvoiceStatus } from '../types';
...
const Reports: React.FC<ReportsProps> = ({ transactions, invoices, customers }) => {
    ...
    const profitLossSummary = useMemo(() => {
        ...
        // Calculate income directly from paid invoices within the date range.
        // This aligns user expectation that a "paid" invoice is considered income.
        const income = invoices
            .filter(i => i.status === InvoiceStatus.Paid && i.issueDate >= start && i.issueDate <= end)
            .reduce((sum, invoice) => {
                const total = invoice.items.reduce((itemSum, item) => itemSum + item.quantity * item.price, 0);
                return sum + (total - invoice.discount);
            }, 0);

        // Calculate expenses from transactions within the date range.
        const expense = transactions
            .filter(t => t.type === TransactionType.Expense && t.date >= start && t.date <= end)
            .reduce((sum, t) => sum + t.amount, 0);
        
        return { income, expense, netProfit: income - expense };
    }, [transactions, invoices, timePeriod]);
    ...
    const revenueByCustomer = useMemo(() => {
        ...
        invoices
            .filter(invoice => invoice.issueDate >= start && invoice.issueDate <= end)
            .forEach(invoice => {
        ...
        return Object.values(customerRevenue).sort((a,b) => b.revenue - a.revenue).slice(0, 10); // Top 10
        ...
        <div className="space-y-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-200/50">
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 text-center">
                    <div>
                        <p className="text-sm font-medium text-slate-500">درآمد کل</p>
                        <p className="text-3xl font-bold text-green-600 mt-2">{formatCurrency(profitLossSummary.income)}</p>
                    </div>
                    <div>
                        <p className="text-sm font-medium text-slate-500">هزینه کل</p>
                        <p className="text-3xl font-bold text-red-600 mt-2">{formatCurrency(profitLossSummary.expense)}</p>
                    </div>
                    <div>
                        <p className="text-sm font-medium text-slate-500">سود خالص</p>
                        <p className={`text-3xl font-bold mt-2 ${profitLossSummary.netProfit >= 0 ? 'text-slate-800' : 'text-red-600'}`}>
                        ...
                        <div className="flex flex-col md:flex-row items-center gap-8">
                            <div className="relative w-40 h-40">
                                <svg className="w-full h-full" viewBox="0 0 36 36" transform='rotate(-90)'>
                                    {expensesByCategory.reduce((acc, cat, index) => {
                                        const offset = acc;
                                        const dasharray = `${cat.percentage}, 100`;
                                        return offset + cat.percentage;
                                    }, 0) && expensesByCategory.map((cat, index, arr) => {
