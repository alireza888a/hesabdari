...
const Products: React.FC<ProductsProps> = ({ products, onAdd, onUpdate, onDelete, onDeleteMultiple }) => {
  ...
  const handleFormSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const finalData = { ...formData, price: Number(formData.price), costPrice: Number(formData.costPrice) };
    if (editingProduct) {
    ...
    <>
      <div className="bg-white p-6 rounded-2xl shadow-md border border-slate-200/50">
        ...
        <div className="space-y-2">
            <div className="hidden md:grid grid-cols-[auto,50px,2fr,1fr,1fr,1.5fr,100px] gap-4 p-4 text-sm font-semibold text-slate-500">
                ...
                <span className="text-left">قیمت خرید</span>
                <span className="text-left">قیمت فروش</span>
                <span>توضیحات</span>
                <span className="text-center">عملیات</span>
            </div>
             {filteredProducts.map(product => (
                <div key={product.id} className="grid grid-cols-[auto,1fr,auto] md:grid-cols-[auto,50px,2fr,1fr,1fr,1.5fr,100px] items-center gap-4 p-4 bg-slate-50 rounded-2xl hover:shadow-sm transition-shadow">
                    ...
                    <div className="hidden md:block text-sm text-slate-600 font-medium text-left">{formatCurrency(product.costPrice)}</div>
                    <div className="hidden md:block text-sm text-slate-800 font-bold text-left">{formatCurrency(product.price)}</div>
            ...
            <form onSubmit={handleFormSubmit}>
              <div className="p-6 space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label htmlFor="costPrice" className="block text-sm font-semibold text-slate-700 mb-1">قیمت خرید (تومان)</label>
                      <input type="number" name="costPrice" id="costPrice" value={formData.costPrice} onChange={handleFormChange} required className="w-full p-2.5 border border-slate-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500" />
                    </div>
                    <div>
                      <label htmlFor="price" className="block text-sm font-semibold text-slate-700 mb-1">قیمت فروش (تومان)</label>
